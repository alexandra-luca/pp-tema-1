...
; adăugați primul pipe în starea jocului. Acesta se va afla inițial în afara ecranului.
...
(define-struct teava (x y) #:transparent)
...
;TODO 3
; Trebuie să implementăm logica gravitației. next-state-bird va primi drept
...
(define (next-state-bird bird gravity)
  (match-let ([(pasare _ by bvy) bird])
    (struct-copy pasare bird [y (+ by bvy)] [vy (+ bvy gravity)])))
...
(define (next-state-bird-onspace bird momentum)
  (struct-copy pasare bird [vy (- 0 momentum)]))
...
(define (change current-state pressed-key)
  (if (key=? pressed-key " ")
      (struct-copy stare current-state [p (next-state-bird-onspace (stare-p current-state) initial-momentum)])
      (current-state)))
...
; scroll-speed-ul dat.
(define (move-one-pipe pipe scroll-speed)
  (struct-copy teava pipe [x (- (teava-x pipe) scroll-speed)]))

(define (move-pipes pipes scroll-speed)
  (map (λ (t) (move-one-pipe t scroll-speed)) pipes))
